...
let W=3000,H=1800,started=false,screen="menu",cam={x:1500,y:900},keys={},mouse={x:innerWidth/2,y:innerHeight/2,down:false},lastShot=0,effects=[];
...
if(m.type==="left"){screen="menu";started=false;result.hidden=true;lobby.hidden=true;hud.hidden=true;menu.hidden=false;send({type:"rooms"});}
if(m.type==="joined"){me.id=m.id;mapName=m.map;W=m.world.W;H=m.world.H;screen="lobby";menu.hidden=true;result.hidden=true;lobby.hidden=false;hud.hidden=true;q("roomTitle").textContent="ROOM #"+m.code+" — "+mapName}
if(m.type==="players"){sync(m.players);drawMembers()}
if(m.type==="start"){started=true;screen="game";result.hidden=true;lobby.hidden=true;hud.hidden=false;sync(m.players);me.hp=100;me.ink=100}
...
if(m.type==="result"){started=false;screen="result";hud.hidden=true;lobby.hidden=true;showResult(m.players)}
...
q("startBtn").onclick=()=>send({type:"start"});
q("backBtn").onclick=()=>send({type:"leave"});
q("again").onclick=()=>{screen="lobby";result.hidden=true;lobby.hidden=false;hud.hidden=true;send({type:"ready"});};
q("rematch").onclick=()=>send({type:"restart"});
...
if(mouse.down)shoot();if(performance.now()-lastState>50){lastState=performance.now();send({type:"state",x:me.x,y:me.y,a:me.a,hp:me.hp,ink:me.ink,weapon:me.weapon});}cam.x+=(me.x-cam.x)*.12;cam.y+=(me.y-cam.y)*.12;
