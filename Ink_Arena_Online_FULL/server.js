...
wss.on("connection",ws=>{
 ...
 ws.on("message",raw=>{
  ...
  if(!p.room)return;
  const r=rooms.get(p.room); if(!r)return;
  if(m.type==="start" && r.players.get(p.id)){
   r.started=true;r.time=180;
   for(const q of r.players.values()){q.hp=100;q.alive=true;q.score=0;q.special=0;q.ink=100;const s=spawn(q.team);q.x=s.x;q.y=s.y}
   broadcast(r,{type:"start",players:snapshot(r),time:r.time});return;
  }
  if(m.type==="state"){
